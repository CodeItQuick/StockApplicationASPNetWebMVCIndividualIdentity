...
public class ShortlistStockInfoDataServiceTests
{
    ...
    [Fact]
    public void GivenShortListedStocksCanHandleEmptyListOfStocksUsingDatabaseMethods()
    {
        var databaseMethods = new DatabaseMethods();
        var unitOfWork = databaseMethods.CreateTestUnitOfWork();  
        var service = new ShortlistStockInfoDataService(unitOfWork);
        
        var allStocks = service
            .ShortlistedStocks(1, "");
        
        Assert.Equal(0, allStocks.ToList().Count());
